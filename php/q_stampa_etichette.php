...
<html>
<head>
  <title>Stampa etichette Viaggio/Pellegrinaggio</title>
...
/****************************************************************************************************
*
*  Stampa le etichette per il viaggio/pellegrinaggio
*
*  @file q_stampa_etichette.php
*  @abstract Stampa le etichette per il viaggio/pellegrinaggio
*  @author Luca Romano
*  @version 1.0
*  @time 2017-08-02
...
define("ROWPERPAGE", 6); // Righe per pagina
...
$redirect="../php/q_stampa_etichette.php";
$print_target="../php/stampa_etichette.php";
$titolo='Sconosciuto';
...
$titolo = 'Stampa etichette viaggio/pellegrinaggio';
...
disegna_menu();
  ...
  echo "<td><select class='required' name='id_prn' required onChange='this.form.submit();'>" ;
  ...
  if($sqlid_attpell > 0) {
      echo "<form action='" . $print_target . "' target='_new' method='POST'>";
      echo "<input type='hidden' name='id_sottosezione' value='" . $sqlid_sottosezione . "'>";
      echo "<input type='hidden' name='anno' value=$sqlanno>";
      echo "<input type='hidden' name='id_prn' value=$sqlid_attpell>";
  // Permetto di partire da una determinata riga per recuperare i fogli parzialmente utilizzati
  
      echo "<tr>";
      echo "<td><p class='search'>Riga da cui iniziare la stampa</td>";
      echo "<td><select class='search' name='riga'>";
  ...
  // elenco partecipanti (per poter stampare le singole etichette)
      $sql = "SELECT DISTINCT anagrafica.id, CONCAT(anagrafica.cognome,' ',anagrafica.nome) nome
                   ...
                   ORDER BY 2";
      echo "<tr>";
      echo "<td><p class='search'>Partecipante</p></td>";
      echo "<td><select class='search' name='id_socio'>" ;
      echo "<option value=0>--- Tutti i partecipanti ---</option>";
      ...
      echo "<tr>";
      echo  "<td colspan='2' class='button'><p><input class='in_btn' id='btn' type='submit' value='Stampa'></p></td>";
      echo "</tr>";
      echo "</form>";
     }
