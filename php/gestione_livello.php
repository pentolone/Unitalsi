...
<html>
<head>
  <title>Gestione livello utenti applicativo</title>
...
/****************************************************************************************************
*
*  Gestione della descrizione dei livelli utenti
*
*  @file gestione_livello.php
*  @abstract Gestisce la descrizione del livello utente
*  @author Luca Romano
*  @version 1.0
*  @time 2017-10-08
...
$index=0;
$update = false;
$table_name="livello_utente";
$redirect="../php/gestione_livello.php";
...
if(!$multisottosezione) {
   $sqlselect_sottosezione .= " WHERE id_sottosezione = " . $sqlid_sottosezione; 
 }
$sqlselect_sottosezione .= " ORDER BY 2";
$sqlselect_livello = "SELECT 0, id,  descrizione, id_sottosezione
                                  FROM   livello_utente
                                  WHERE id_sottosezione = $sqlid_sottosezione"; 
if($multisottosezione) {
	$sqlselect_livello .= " UNION
	                                  SELECT 1, livello_utente.id,
	                                               CONCAT(livello_utente.descrizione, ' (Sottosezione di ' , sottosezione.nome, ')') descrizione,
	                                               id_sottosezione
                                  FROM   livello_utente,
                                              sottosezione
                                  WHERE livello_utente.id_sottosezione != $sqlid_sottosezione
                                  AND     livello_utente.id_sottosezione = sottosezione.id"; 
 }
$sqlselect_livello .= " ORDER BY 1, 4, 3";
...
// Fine verifica abilitazioni
  ...
  if ($_POST) { // se post allora fase di modifica
      ...
      foreach ($_POST as $key => $value) {
       ...
       $sql = "SELECT descrizione, note, id_sottosezione,
                                 DATE_FORMAT((data), '" .$date_format . "') data,
                                 utente FROM livello_utente WHERE id = " . $sqlID;
...
// verifica autorizzazioni  
  ...
  echo "<table>";
  echo "<tr>";
  echo "<td colspan='2' class='titolo'>Gestione Livello utenti applicativo</td>";
  echo "</tr>";

// Campo per la ricerca del comune
  echo "<form name='searchTxt' action='" . $redirect . "' method='POST'>";
  echo "<tr>";
  echo "<td><p class='search'>Seleziona Livello</p></td>";
  ...
  echo "<tr>";
  echo "<td><p class='required'>Descrizione livello</p></td>";
