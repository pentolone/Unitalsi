...
<html>
<head>
  <title>Gestione ricevute eliminate</title>
...
/****************************************************************************************************
*
*  Gestione delle ricevute eliminate (visualizza/cancellazione)
*
*  @file gestione_ricevute_eliminate.php
*  @abstract Gestisce la visualizzazione/cancellazione della tabella ricevute_eliminate
*  @author Luca Romano
*  @version 1.0
*  @time 2018-05-23
...
$table_name="ricevute_eliminate";
$redirect="../php/gestione_ricevute_eliminate.php";
...
$sqlselectanno_ricevuta_eliminata = "SELECT MIN(YEAR(data_ricevuta)) amin
					                                             FROM   ricevute_eliminate WHERE id_sottosezione=?";                                                          

$sqlselect_ricevute_eliminate = "SELECT CONCAT(anagrafica.cognome,' ',anagrafica.nome) nome,
                                                  ricevute_eliminate.id,
                                                  ricevute_eliminate.causale,
                                                  ricevute_eliminate.importo,
                                                  SUBSTR(DATE_FORMAT(ricevute_eliminate.data_ricevuta,'" . $date_format . "'),1,10) data_ricevuta,
                                                  LPAD(n_ricevuta, 5, '0') n_ricevuta,
                                                  ricevute_eliminate.data_ricevuta d_ord,
                                                  tipo_pagamento.descrizione despag,
                                                  ricevute_eliminate.utente,
                                                  DATE_FORMAT(ricevute_eliminate.data, '" . $date_format . "') data,
                                                  ricevute_eliminate.id_attpell,
                                                  ricevute_eliminate.tipo
                                    FROM    anagrafica,
                                                  ricevute_eliminate
                                    LEFT JOIN tipo_pagamento
                                    ON        ricevute_eliminate.id_pagamento = tipo_pagamento.id
                                    WHERE  anagrafica.id = ricevute_eliminate.id_socio
                                    AND      ricevute_eliminate.id_sottosezione =?
                                    AND      YEAR(data_ricevuta) = ?
                                    AND      lower(CONCAT(cognome, ' ', nome)) like lower(?)
                                    AND      ricevute_eliminate.n_ricevuta BETWEEN ? AND ?
                                    ORDER BY 6 DESC LIMIT 0,?";
// Tipo pagamento
$sqlselect_tipopag = "SELECT descrizione
                                    FROM   tipo_pagamento
                                    WHERE id = ?";
...
// Fine verifica abilitazioni
  ...
  if ($_POST) { // se post allora modificato parametri di input
      ...
      foreach ($_POST as $key => $value) {
           ...
           if($sqlID > 0) { // Visualizzazione riga, carico i dati
               $update=true;
               $sql =  "SELECT CONCAT(anagrafica.cognome,' ',anagrafica.nome) nome,
                                          ricevute_eliminate.causale,
                                          ricevute_eliminate.importo,
                                          SUBSTR(DATE_FORMAT(ricevute_eliminate.data_ricevuta,'" . $date_format . "'),1,10) data_ricevuta,
                                          LPAD(ricevute_eliminate.n_ricevuta, 5, '0') n_ricevuta,
                                          ricevute_eliminate.data_ricevuta d_ord,
                             ...
                             ON        ricevute_eliminate.id_pagamento = tipo_pagamento.id
                             WHERE   anagrafica.id = ricevute_eliminate.id_socio
                             AND       ricevute_eliminate.id = $sqlID";
               if($debug)
                  echo "$fname: SQL = $sql";
...
// verifica autorizzazioni  
  if($ctrAuth == 0) { // Utente non abilitato
   ...
   if($sqlID == -1) { // Richiesta cancellazione di tutte le righe
       $sql = "DELETE FROM ricevute_eliminate
  ...
  $conn->prepare($sqlselect_ricevute_eliminate);
  echo "<form name='searchTxt' action='" . $redirect . "' method='POST'>";
  echo "<input type='hidden' name='nrec' value=$nrec>";
  echo "<table>";
  echo "<tr>";
  echo "<td colspan='2' class='titolo'>Gestione Ricevute ELIMINATE";
  if($update) {
...
// Se aggiorno visualizzo i dati di dettaglio della ricevuta

  // tipo pagamento
  $stmt = $conn->prepare($sqlselect_tipopag);
  $stmt->bind_param("i", $sqlid_pagamento);
  $stmt->execute();
  $stmt->store_result();
  $stmt->bind_result($sqldes_pagamento);
  $stmt->fetch();
...
// Attivita'/Viaggio
    ...
    echo "<td><p class='required'>" . htmlentities($sqlcausale,$defCharsetFlags, $defCharset) . "</p></td>";
    echo "</tr>";

    echo "<tr>";
    echo "<td><p class='required'>Importo</p></td>";
    echo "<td><p class='required'>" . $sqlimporto . "</p></td>";
    echo "</tr>";

    echo "<tr>";
    echo "<td><p class='required'>Tipo di pagamento</p></td>";
    echo "<td><p class='required'>" . htmlentities($sqldes_pagamento,$defCharsetFlags, $defCharset) . "</p></td>";
    echo "</tr>"; 
    ...
    echo "</tr>";
     ...
     if($authMask["delete"]) {
         ...
         echo "<td class='button'><input class='md_btn' type='submit' value='Elimina' onclick=\"{return conferma('". ritorna_js("Cancello la ricevuta ELIMINATA?") ."');}\"></td>";
         echo "</form>";
   	     } 
   	   echo "</tr></table></td></tr>";
     ...
     if($authMask["delete"]) {
         ...
         echo "<input type=\"hidden\" name=\"id_sottosezione\" value=\"" . $sqlid_sottosezione . "\">";
         echo "<tr>";
         echo "<td colspan=2 style='text-align: center;'><input class='md_btn' type='submit' value='Elimina TUTTE' onclick=\"{return conferma('". ritorna_js("Cancello TUTTA la lista delle ricevute ELIMINATE ?") ."');}\"></td>";
   	      echo "</tr>";
         echo "</form>";
        }
     }
  ...
  // Elenco ultime 'n' ricevute_eliminate
  $index=0;
  $stmt = $conn->prepare($sqlselect_ricevute_eliminate);
...
// Descrizione attivita'/Pellegrinaggio
  ...
  echo "<a href='../php/gestione_ricevute_eliminate.php'><input type='button' value='Indietro'></a>";
  echo "</div>";
