...
<html>
<head>
  <title>Gestione stato Civile</title>
...
/****************************************************************************************************
*
*  Gestione della descrizione dello stato civile
*
*  @file gestione_stato_civile.php
*  @abstract Gestisce la descrizione dello stato civile
*  @author Luca Romano
*  @version 1.0
*  @time 2017-01-23
...
$table_name="stato_civile";
$redirect="../php/gestione_stato_civile.php";
...
$sqlselect_stato_civile = "SELECT id,  descrizione
                                          FROM   stato_civile
                                          ORDER BY 2";
  ...
  if ($_POST) { // se post allora fase di modifica
      ...
      foreach ($_POST as $key => $value) {
       ...
       $sql = "SELECT descrizione, note,
                                 ...
                                 utente FROM stato_civile WHERE id = " . $sqlID;
...
// verifica autorizzazioni  
  ...
  echo "<table>";
  echo "<tr>";
  echo "<td colspan='2' class='titolo'>Gestione Stato civile</td>";
  echo "</tr>";

// Campo per la ricerca del comune
  echo "<form name='searchTxt' action='" . $redirect . "' method='POST'>";
  echo "<tr>";
  echo "<td><p class='search'>Seleziona Stato civile</p></td>";
  ...
  $result = $conn->query($sqlselect_stato_civile);
  ...
  echo "<tr>";
  echo "<td><p class='required'>Descrizione Stato civile</p></td>";
