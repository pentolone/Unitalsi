...
<html>
<head>
  <title>Gestione tipo documento</title>
...
/****************************************************************************************************
*
*  Gestione della tipologia dei documenti
*
*  @file gestione_tipo_documento.php
*  @abstract Gestisce la descrizione della tipologia del documento identificativo
*  @author Luca Romano
*  @version 1.0
*  @time 2017-01-26
...
$redirect="../php/gestione_tipo_documento.php";
...
// Fine verifica abilitazioni
  ...
  if ($_POST) { // se post allora fase di modifica
      ...
      foreach ($_POST as $key => $value) {
       ...
       $sql = "SELECT descrizione, codice_PS, note,
                                 DATE_FORMAT((data), '" .$date_format . "') data,
                                 utente FROM tipo_documento WHERE id = " . $sqlID;
...
// verifica autorizzazioni  
  ...
  echo "<table>";
  echo "<tr>";
  echo "<td colspan='2' class='titolo'>Gestione Tipo documento</td>";
  echo "</tr>";

// Campo per la ricerca del tipo documento
  echo "<form name='searchTxt' action='" . $redirect . "' method='POST'>";
  echo "<tr>";
  echo "<td><p class='search'>Digita Tipo documento</p></td>";
  echo "<td><select class='search' name='id-hidden'  onChange='this.form.submit();'>";
  $result = $conn->query($sqlselect_tipo_doc);
  ...
  echo "<tr>";
  echo "<td><p class='required'>Descrizione Tipo documento</p></td>";
  ...
  echo "<td><p class='required'><input class='required' maxlength='5' size='6' type='value' name='codice_PS' value='" .  htmlentities($sqlcodice_PS, $defCharsetFlags, $defCharset) ."' required/></p></td>";
  echo "</tr>";
  ...
  if($update) {
     ...
     if($authMask["insert"]) { // Visualizzo pulsante solo se abilitato
          echo  "<tr><td colspan='2' class='button'><p><input class='in_btn' id='btn' type='submit' value='Inserisci'></p></form></td>";
          echo "</tr>";
         }
     }
  echo "</table>";
