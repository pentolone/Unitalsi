...
<html>
<head>
  <title>Anagrafica luoghi viaggi/pellegrinaggi</title>
...
/****************************************************************************************************
*
*  Gestione dei luoghi viaggi/pellegrinaggi
*
*  @file gestione_luoghi.php
*  @abstract Gestisce l'anagrafica dei luoghi/pellegrinaggi
*  @author Luca Romano
*  @version 1.0
*  @time 2017-02-13
...
$table_name="luoghi";
$redirect="../php/gestione_luoghi.php";
...
$sqlselect_luogo = "SELECT 0, id,  descrizione
                                 FROM   luoghi
                                 WHERE  id_sottosezione = " . $sott_app; 
 
 if($multisottosezione) {
        $sqlselect_luogo .= " UNION
                                          SELECT 1, luoghi.id,
                                          CONCAT(luoghi.descrizione,' (Sottosezione di ' , sottosezione.nome,')') descrizione
                                          FROM    luoghi,
                                                       sottosezione
                                          WHERE  luoghi.id_sottosezione = sottosezione.id
                                          AND      id_sottosezione != " . $sott_app; 
 }
$sqlselect_luogo .= " ORDER BY 1,3";
...
// verifica autorizzazioni  
  ...
  echo "<table>";
  echo "<tr>";
  echo "<td colspan='2' class='titolo'>Gestione Luoghi viaggio/pellegrinaggio</td>";
  echo "</tr>";

// Campo per la ricerca del luogo
  echo "<form name='searchTxt' action='" . $redirect . "' method='POST'>";
  echo "<tr>";
  echo "<td><p class='search'>Seleziona Luogo</p></td>";
  ...
  echo "<tr>";
  echo "<td><p class='required'>Luogo</p></td>";
