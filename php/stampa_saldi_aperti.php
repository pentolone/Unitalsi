...
/****************************************************************************************************
*
*  Stampa i saldi aperti dei partecipanti ad attivita'/viaggio/pellegrinaggio
*
*  @file stampa_saldi_aperti.php
...
if(!$_POST) { // se NO post allora chiamata errata
   ...
   return;
    ...
    foreach ($_POST as $key => $value) {
                  $kv[] = "$key=$value";
                   ...
                   switch($key) {
      		                      ...
      		                      case "id_sottosezione": // id_sottosezione
      		                       ...
      		                       case "tipo": // 'A' = Attivita 'V' = Viaggio/Pellegrinaggio 'T' = Entrambi
      					                      $sqltipo= $value;
      					                      switch($sqltipo) {
      					                      	           case 'A': // solo attivita'
      					                      	                        $condSel = " = 'A'";
      					                      	                        break;
             ...
             $txtF = "Stampa saldi aperti (Rev 1.0) Generato il: " . date('d/m/Y H:i:s'); 
...
$sqlP = "SELECT CONCAT(anagrafica.cognome,' ',anagrafica.nome) nome,
                            ...
                            (SUM(costi_detail.costo*costi_detail.qta) - SUM(costi_detail.valore)) dovuto,
                            SUM((costi_detail.costo*costi_detail.qta)-costi_detail.valore-ricevute.importo) saldo
              FROM     anagrafica,
                            costi_detail,
                            pellegrinaggi,
                            ...
                            attivita_detail
                            LEFT JOIN ricevute
                            ON         (ricevute.id_socio = attivita_detail.id_socio
                            AND       ricevute.id_attpell = attivita_detail.id_attpell 
                            AND       ricevute.tipo = attivita_detail.tipo)
              WHERE   attivita_detail.tipo " . $condSel . "
